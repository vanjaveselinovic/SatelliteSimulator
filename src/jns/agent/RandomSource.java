...
public class RandomSource implements Agent {
	 ...
	 * 
	 * @param source
	 *            the source IP from which to send (one of the IP addresses of the
	 *            node this is at).
	 * @param dest
	 *            the IP to send to. Make sure this IP has a RandomSink attached and
	 *            listening before you start sending!
	 * @param start_time
	 *            the time at which to start sending
	 * @param stop_time
	 *            the time at which to stop sending
	 */
	public RandomSource(IPAddr source, IPAddr dest, double start_time, double stop_time) {
		m_source_ip = source;
		m_dest_ip = dest;
		...
		Simulator.getInstance().schedule(new Command("RandomSourceStart", m_start) {
			public void execute() {
		...
		Simulator.getInstance().schedule(new Command("RandomSourceStop", m_end) {
			public void execute() {
	...
	public void indicate(int status, Object indicator) {
		if (status == Agent.CONNECTION_ESTABLISHED) {
			...
			Simulator.getInstance()
					.schedule(new RandomSourceSendCommand(Simulator.getInstance().getTime() + 0.001, this));
		}
	}

	/**
	 * RandomSource is supposed to be a top-level agent. It sends data by itself and
	 * no one is supposed to use it to send data. Thus this function will always
	 * return 'false'.
	 * 
	 * @param destination
	 *            ignored
...
class RandomSourceSendCommand extends Command {
	...
	public void execute() {
		...
		if (Simulator.getInstance().getTime() + 0.005 < m_source.m_end)
			Simulator.getInstance()
					.schedule(new RandomSourceSendCommand(Simulator.getInstance().getTime() + 0.005, m_source));
	}
