...
public class SimpleGoBackN implements CL_Agent {
	 ...
	 * source port. This basically takes care of setting up the agent, attaching it
	 * to this class, etc. You can just go ahead and send data afterwards (after
	 * connecting, etc.)
	 * 
	 * @param local_port
	 *            the port at which to open the agent. If you use a duplicate port
	 *            you will get a simulator error displayed.
	 */
	...
	/**
	 * Attach callback for lower level agents. SimpleGoBackN can only run on top of
	 * IP.
	 * 
	 * @param lower_level
	...
	public void indicate(int status, Object indicator) {
		if (status == Agent.READY_TO_SEND) {
			for (Enumeration e = m_ports.elements(); e.hasMoreElements();)
				((AgentPort) e.nextElement()).agent.indicate(Agent.READY_TO_SEND, this);
			...
			AgentPort agentport = (AgentPort) m_ports.get(new Integer(packet.destination_port));
	...
	/**
	 * This agent will always accept packets from the higher level and buffer them.
	 */
	...
	public Object read(int unique_id) {
		...
		if (port == null)
			Simulator.error("SGN: Confused; someone who's not attached is reading" + "from a port");
