...
/**
 * This is a (reasonably) simple protocol implementation that uses a 'go-back-n'
 * strategy for providing a error-correcting connection-oriented service. If an
 * acknowledgement does not arrive in time, all packets from that ACK on are
 * sent again. This particular implementation actually provides a two-way
 * service, data transfer in both directions.
 * <p>
 * In order to handle connection setup and teardown properly, I chose to use the
 * TCP finite state machine. The state machine is implemented by having the
 * current state in a variable and acting depending on the value of this
 * variable in the different functions.
 * <p>
 * For an overview of the TCP finite state machine, please refer to the
 * "Internetworking with TCP/IP" book series. You can find the diagram both in
 * Volume I ("Principles, Protocols and Architectures") and Volume II ("Design,
 * Implementation and Internals"). In any case, every good TCP/IP book should
 * have the finite state machine in it.
 * <p>
 * 
...
public class SimpleGoBackNAgent implements CO_Agent {
	 ...
	 * constructor is private to the outside world. If you want a new agent, use
	 * jns.agent.SimpleGoBackN.createNewAgent. This function will also take care of
	 * attaching everything correctly.
	 */
	 ...
	 *            the agent to attach
	 * @param unique_id
	 *            an id that will identify the agent
	 * @see jns.agent.Agent
	 */
	...
	public void attach(Agent lower_level) {
		if (!(lower_level instanceof SimpleGoBackN))
			Simulator.error("You may attach SimpleGoBackNAgent only to " + "SimpleGoBackN");

		m_sgn = (SimpleGoBackN) lower_level;
	}

	/**
	 * Set up a connection with a remote host. All the details can be found in the
	 * CO_Agent class. Calling this function initiates a three-way handshake. Note
	 * that it takes some time to do the handshake.
	 * 
	 * @see jns.agent.CO_Agent
	 */
	public void connect(IPAddr source, IPAddr destination, int destination_port) {
		if (m_state != CLOSED) {
			Simulator.warning("Call to SGN connect() ignored because the protocol" + " was not in the idle state");
			return;
		}
	 ...
	 * connection-teardown to be initialised. Note that it will take some time to
	...
	public void disconnect() {
		...
		default:
			Simulator.warning("Disconnect already in progress " + "when called in SGN");
		}
	}

	/**
	 * Start listening at a specific IP address. The port to listen at has already
	 * been specified when the agent was created.
	 * 
	 * @param local_ip
	...
	public void listen(IPAddr local_ip) {
		if (m_state != CLOSED) {
			Simulator.warning("Call to SGN listen() ignored because the protocol " + "was not in the idle state");
			return;
		}
	...
	public void send(int length, Object data, int unique_id) {
		if (m_state != ESTABLISHED) {
			Simulator.warning(
					"Tried to send a packet using SGN without setting " + "up a connection first. Packet LOST!");
			return;
		}
		...
		Simulator.getInstance().schedule(new SGNUpdateCommand(Simulator.getInstance().getTime() + 0.0001, this));
	}
	...
	/**
	 * This is an agent that serves the application layer. We will thus always allow
	 * packets to be sent for convenience and buffer them ourselves.
	 * 
	 * @param destination
	 *            ignored
	 * @param length
	...
	public void indicate(int status, Object indicator) {
		if (status == Agent.READY_TO_SEND) {
			...
			if (m_send_array.size() > 0 && m_window_pos < m_window_size && m_window_pos < m_send_array.size()) {
				Simulator.getInstance()
						.schedule(new SGNUpdateCommand(Simulator.getInstance().getTime() + 0.0001, this));
			}
			...
			switch (m_state) {
			case CLOSED:
				Simulator.warning("SGN received a packet in the closed" + " state. Ignored.");
				break;
			...
			case LISTEN:
				if (packet.flags == SimpleGoBackNPacket.SYN) {
					...
					sendPacket(SimpleGoBackNPacket.SYN | SimpleGoBackNPacket.ACK, m_seq_data, packet.sequence);
					m_state = SYN_RECVD;
				}
				break;
			...
			case SYN_SENT:
				if (packet.flags == SimpleGoBackNPacket.SYN) {
					sendPacket(SimpleGoBackNPacket.SYN | SimpleGoBackNPacket.ACK, m_seq_data, packet.sequence);
					m_state = SYN_RECVD;
				} else if (packet.flags == (SimpleGoBackNPacket.SYN | SimpleGoBackNPacket.ACK)) {
					sendPacket(SimpleGoBackNPacket.ACK, m_seq_data, packet.sequence);
					m_state = ESTABLISHED;
			...
			case ESTABLISHED:
				...
				if (packet.flags == SimpleGoBackNPacket.ACK && m_send_array.size() > 0) {
					if (packet.ack == ((SimpleGoBackNPacket) m_send_array.elementAt(0)).sequence) {
			...
			case FIN_WAIT1:
				if (packet.flags == SimpleGoBackNPacket.FIN) {
					...
					m_state = FIN_WAIT2;
				} else if (packet.flags == (SimpleGoBackNPacket.FIN | SimpleGoBackNPacket.ACK)) {
					sendPacket(SimpleGoBackNPacket.ACK, m_seq_data, packet.sequence);
					m_state = TIME_WAIT;
				}
				break;
	...
	/**
	 * Internal function to which calls are scheduled with the simulator. On each
	 * call, update will try to send a packet (if the window size and ACKs permit
	 * it)
	 */
	protected void update() {
		if (m_send_array.size() > 0 && m_window_pos < m_window_size && m_window_pos < m_send_array.size()) {
			SimpleGoBackNPacket packet = (SimpleGoBackNPacket) m_send_array.elementAt(m_window_pos);
	...
	/**
	 * sendPacket is used to send control packets (SYN, etc.). Those packets contain
	 * no data.
	 */
