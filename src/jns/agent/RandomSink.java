...
public class RandomSink implements Agent {
	...
	/**
	 * Create a new random sink that will start listening for incoming connections
	 * at some point and stop listening at some other point.
	 * 
	 * @param ip
	 ...
	 */
	public RandomSink(IPAddr ip, double start_time, double stop_time) {
		m_local_ip = ip;
		m_remote_ip = null;
		...
		Simulator.getInstance().schedule(new Command("RandomSinkStart", m_start) {
			public void execute() {
	...
	/**
	 * This is a 'sink' that is there to receive packets, not send them. Thus this
