...
/**
 * This is the main class used to dynamically schedule events to the JNS network
 * simulator. It will start the *modified* simulator, and then any event that
 * gets passed to it will be scheduled with it. This will allow (hopefully) for
 * "real" implementations of network protocols to be simulated for correctness.
 *
 * To use this class, you need to add the nodes and links to the scheduler in
 * the main() function below, alternatively you can write your own main()..
...
public class DynamicSchedulerImpl extends UnicastRemoteObject implements DynamicScheduler, Agent {
	...
	/**
	 * This is a reference to the JNS simulator instance. This needs to get passed
	 * to the
	 */
	...
	/**
	 * This is a hashmap of message queues mapped onto the IP address of the node
	 * which sent it.
	 */
	 ...
	 *
	 * @param nameOfRun
	 *            The name of this simulation run. Gives name to the JVS file.
	 * @param linkBandwidth
	 *            The bandwidth of all the links between the nodes (could change
	 *            this to set it for each link added)
	 * @param linkDelay
	 *            The link delay of all the links between the nodes (could change
	 *            this to set it for each link added)
	 * @param linkErrorRate
	 *            The link error rate of all the links between the nodes (could
	 *            change this to set it for each link added)
	 * @param subnetMask
	 *            The subnet mask of all the routes given to the nodes (could change
	 *            this to set it for each link added)
	 */
	public DynamicSchedulerImpl(String nameOfRun, int linkBandwidth, double linkDelay, double linkErrorRate,
			IPAddr subnetMask) throws RemoteException {
	...
	/**
	 * Adds a node to the simulator. The IP address is used as both name and IP
	 * address. This method can currently not be called remotely
	 */
	...
	/**
	 * Adds a link between the two nodes. Beware that this is a two way link, so
	 * there is no need to add a link "the other way" This method can currently not
	 * be called remotely
	 *
	 * @param nodeA
	 *            The IP address of the first node to connect
	 * @param nodeB
	 *            The IP address of the second node to connect
	 */
	public void addLink(IPAddr nodeA, IPAddr nodeB) {
		...
		if (a == null || b == null) {
			System.err.println(
					"Trying to add a link between two nodes (" + linkName + "), one or more which does not exits.");
			return;
		}
	 ...
	 */
	public synchronized void scheduleUnicast(InetAddress senderIPAddress, InetAddress receiverIPAddress, byte[] data)
	 ...
	 */
	public synchronized void scheduleMulticast(InetAddress senderIPAddress, InetAddress multicastGrp, byte[] data)
			throws RemoteException {
		IPAddr senderIPAddr = new IPAddr(senderIPAddress);
	...
	/**
	 * Receives the next message for the IPAddr indicated, will block if untill
	 * there is a message available
	 *
	 * TODO: return more information that might be required to properly set a
	 * datagram.
	 */
	...
	/**
	 * This method will stop the simulator. TODO: Change this so the simulator does
	 * not stop untill all the participants have called stop()
	 */
	...
	/**
	 * This method will start the simulator. This is called by the main() method in
	 * this class.
	 */
	...
	// -----------------------------------------
	/**
	 * Normally, this would attach a higher level agent to this agent interface.
	 * However, as the dynamic scheduler propagates all its packets to external
	 * clients, trying to attach a higher level agent does not make any sense. So
	 * calling this function will result in a Simulator error.
	 *
	 */
	public void attach(Agent higher_level, int unique_id) {
		Simulator.error(
				"A higher level agent tried to attach itself to the dynamic scheduler. This makes no sense. See documentation.");
	}

	/**
	 *
	 * Attach a lower-level agent to this agent. This function is used as a
	 * callback. When a higher-level agent attaches itself to a lower level agent,
	 * the lower level agent will attach itself to the higher-level agent using this
	 * function.<br>
	 * The purpose of this is that the higher-level agent will implement this
	 * function in order to store some reference to the lower-level service which it
	 * can use to give packets to it.<br>
	 * Do NOT call this function as a user of the agent, only agents between
	 * themselves need to use it.
	 *
	 * In the dynamic scheduler, this function gets called by the node's IPHandler
	 * when the the dynamic scheduler gets attached to the IPHandler. This is so
	 * that packets can be passed up to the dynamic scheduler
	 *
	 * @param lower_level
	 *            the lower level agent to attach to this agent.
	 */
	...
	/**
	 * Indicate to this agent that a packet is waiting for collection. This is used
	 * by lower-level agents to indicate to higher-level agents. Note that
	 * higher-level agents may ignore any of the requests!
	 * 
	 * @param status
	 *            one of the constants defined above.
	 * @param indicator
	 *            the object that is indicating, normally an Agent, but could be an
	...
	/**
	 * Query this agent if a packet of a given length could be sent. You must not
	 * call the 'send' function of any agent before calling this function. There are
	 * two possibilities:
	 * <ul>
	 * <li>The function returns 'true' - Go ahead and call 'send'
	 * <li>The function returns 'false' - In this case you are guaranteed to get a
	 * READY_TO_SEND indication by the lower level agent you are using. Do not try
	 * to send any packets but wait for a call to your indicate function with that
	 * flag set. When you do get that call, you <b>must</b> call canSend() again
	 * because someone else might take their turn before you!
	 * </ul>
	 * 
	 * @param destination
	...
	public static void main(String[] args) {
		...
		try {
			...
			DynamicSchedulerImpl scheduler = new DynamicSchedulerImpl("mySimulationRun", 500000, 0.008, 0.0,
					new IPAddr(255, 255, 255, 0));
			...
			/*
			 * scheduler.addNode(new IPAddr(192, 168, 0, 4)); scheduler.addNode(new
			 * IPAddr(192, 168, 0, 5)); scheduler.addNode(new IPAddr(192, 168, 0, 6));
			 * scheduler.addNode(new IPAddr(192, 168, 0, 7)); scheduler.addNode(new
			 * IPAddr(192, 168, 0, 8)); scheduler.addNode(new IPAddr(192, 168, 0, 9));
			 */
