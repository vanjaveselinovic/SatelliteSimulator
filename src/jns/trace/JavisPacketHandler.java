...
public class JavisPacketHandler extends JavisHandler {
	...
	/**
	 * When a new Handler is created a hashtable with all the types of parameters
	 * that can occur in this event is added. The key is the name of the parameter
	 * and the value an integer that is later used in a switch statement thus
	 * enabling an appropriate response to each parameter.
	 */
	...
	/**
	 * Looks at the event and outputs a line of text to the trace file describing
	 * the event in Javis format
	 */
	public void handleEvent(Event e, BufferedWriter w) throws IOException {
		...
		if (e.getName().equals("HopEvent"))
			...
			w.write("d ");
		else if (e.getName().equals("LinkDropEvent"))
			w.write("d ");
