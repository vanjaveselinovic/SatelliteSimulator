...
public class JavisTrace extends Trace {
	...
	/**
	 * Create a new Javis trace object that will write to the file with the supplied
	 * name. If the name ends with ".gz", then the output will be written in
	 * compressed form (Javis is capable of reading compressed input, NAM isn't, so
	 * beware). Compressed output is recommended because trace-files do get big.
	 */
	public JavisTrace(String filename) throws IOException {
		...
		if (filename.endsWith(".gz"))
			m_writer = new BufferedWriter(new OutputStreamWriter(new GZIPOutputStream(new FileOutputStream(filename))));
		else
			m_writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(filename)));
	}
	...
	public void writePreamble() {
		...
		try {
			m_writer.write("# Trace file generated by JNS Version " + Preferences.VERSION + ".");
			m_writer.newLine();
		...
		while (e.hasMoreElements()) {
			...
			if (curelement instanceof Node) {
				...
				try {
					m_writer.write("l -t * ");
					m_writer.write("-s " + link.getOutgoingInterface().getNode().getNumber() + " ");
					m_writer.write("-d " + link.getIncomingInterface().getNode().getNumber() + " ");
					m_writer.write("-S UP ");
