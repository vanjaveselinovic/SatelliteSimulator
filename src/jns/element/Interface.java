...
public abstract class Interface extends Element implements Agent {
	 ...
	 * default. Bandwidth will be zero and the MTU taken from the Preferences class.
	 * 
	 * @param addr
	...
	/**
	 * This constructor will generate a new interface with all values set to default
	 * and with the bandwidth set from the parameter.
	 * 
	 * @param addr
	 *            the IP address of this interface
	 * @param bandwidth
	 *            the bandwidth this interface will have
	 * @see jns.util.Preferences
	 */
	...
	/**
	 * Attach a link to this interface. The bandwidth of the link can be inherited
	 * optionally.
	 * 
	 * @param link
	 *            the link to attach to this interface
	 * @param inheritBandwidth
	 *            if true, this interface's bandwidth will be overridden by the
	 *            link's.
	 */
	...
	/**
	 * getType returns the type of this interface, i.e. SENDER or RECEIVER, or both.
	 */
	...
	public void setMTU(int mtu) {
		if (mtu < 0)
			Simulator.error("Tryed to set an MTU smaller than zero!");
		if (mtu < 16)
			Simulator.warning("You are setting an MTU smaller than the " + "standard IP header");
		m_mtu = mtu;
	}

	/**
	 * Return the maximum transfer unit of this interface in bytes. (That's BYTES,
	 * not bits, careful).
	 */
	public int getMTU() {
		return m_mtu;
