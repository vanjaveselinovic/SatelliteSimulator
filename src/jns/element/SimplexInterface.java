...
public class SimplexInterface extends Interface {
	 ...
	 * schedules a call to the update() method. You are advised to use the canSend
	 * function to check if the packet can be sent first, otherwise it might be
	 * dropped.
	 * 
	 * @param packet
	...
	public void send(IPPacket packet) {
		...
		Simulator.getInstance().schedule(new ElementUpdateCommand(this,
				Simulator.getInstance().getTime() + Preferences.delay_iface_send_update));
	...
	public Object read(int unique_id) {
		...
		if (packet == null) {
			Simulator.warning("No packets in queue when read() was called - " + "Natural Overflow?");
			return null;
		}
	...
	/**
	 * update will do either of two things: If the interface is a SENDER, it will
	 * try to take packets off the queue and put them on the link. Conversely, if it
	 * is a receiver, it will take packets off the link and stick them in the queue.
	 * WRONG.
	 */
	public void update() {
		...
		if (m_link == null) {
			System.err.println("SIMULATOR ERROR: You are trying to send packets " + "from an iface without a link!");
			System.exit(1);
		}
